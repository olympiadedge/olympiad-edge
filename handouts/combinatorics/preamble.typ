...
#set document(title: "Combinatorics — Olympiad Edge", author: "Olympiad Edge")
#set page(
  paper: "us-letter",
  ...
  header: context {
    if counter(page).get().first() > 2 {
      ...
      grid(
        columns: (1fr, 1fr),
        align(left)[_Olympiad Edge — Combinatorics_],
...
#let amc(year, id, body) = block(
  width: 100%, fill: luma(248), stroke: (left: 3pt + gold), inset: (left: 8pt, y: 4pt), radius: (right: 3pt),
  [#text(weight: "bold", fill: navy, size: 10pt)[AMC #year #id] #h(1fr) #text(fill: gold, size: 8pt, weight: "bold")[COMPETITION] #v(2pt) #body]
)

#let aime(year, id, body) = block(
  width: 100%, fill: luma(248), stroke: (left: 3pt + red), inset: (left: 8pt, y: 4pt), radius: (right: 3pt),
  [#text(weight: "bold", fill: navy, size: 10pt)[AIME #year \##id] #h(1fr) #text(fill: red, size: 8pt, weight: "bold")[COMPETITION] #v(2pt) #body]
)

#let putnam(year, id, body) = block(
  width: 100%, fill: luma(248), stroke: (left: 3pt + purple), inset: (left: 8pt, y: 4pt), radius: (right: 3pt),
  [#text(weight: "bold", fill: navy, size: 10pt)[Putnam #year #id] #h(1fr) #text(fill: purple, size: 8pt, weight: "bold")[COMPETITION] #v(2pt) #body]
)
...
#let difficulty(level) = {
  ...
  let label = if level == 1 { "AIME" } else if level == 2 { "Olympiad" } else { "Putnam" }
...
#page(header: none, footer: none, margin: (top: 1.8in, bottom: 1in))[
  #align(center)[
    #block(width: 85%, fill: navy, inset: (x: 24pt, y: 28pt), radius: 8pt)[
      #text(fill: gold, size: 30pt, weight: "bold")[COMBINATORICS]
      #v(2pt)
      #text(fill: white, size: 18pt)[Competition Mathematics]
      #v(10pt)
      #line(length: 50%, stroke: 1pt + gold)
      #v(10pt)
      #text(fill: gold, size: 12pt, weight: "bold")[OLYMPIAD EDGE]
      #v(4pt)
      #text(fill: white, size: 10pt)[AIME · USAMO · Putnam · Self-Study Edition]
    ]
    #v(10pt)
    #block(width: 80%, inset: 12pt)[
      #text(size: 10.5pt, fill: navy)[
        From counting to generating functions to the probabilistic method. \
        Every technique gets a one-line TLDR. Escalating hints on every problem. \
        Bijective proofs, double counting, and graph theory treated as first-class tools.
        #v(8pt)
        *Target: AIME → USAMO Bridge / Putnam Top 500*
        #v(6pt)
        #text(size: 9pt, fill: luma(100))[
          Sources: Art of Problem Solving, Evan Chen, Yufei Zhao, \
          Richard Stanley (Enumerative Combinatorics), Herbert Wilf (generatingfunctionology), \
          Pranav Sriram (Olympiad Combinatorics), past AIME/Putnam 2000–2025.
        ]
      ]
    ]
    #v(12pt)
