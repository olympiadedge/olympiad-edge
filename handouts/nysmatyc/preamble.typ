...
#set document(title: "NYSMATYC Speed Handbook — Olympiad Edge", author: "Olympiad Edge")
#set page(
  paper: "us-letter",
  ...
  header: context {
    if counter(page).get().first() > 2 {
      ...
      grid(
        columns: (1fr, 1fr),
        align(left)[_Olympiad Edge — NYSMATYC Speed Handbook_],
...
// SPEED BOX — the signature box for this handbook
#let speedbox(body) = block(
  width: 100%, fill: lightgold, stroke: 1.5pt + gold, radius: 3pt, inset: 8pt,
  [#text(fill: gold, weight: "bold", size: 9.5pt)[FASTEST PATH] #v(2pt) #body]
)

#let trickbox(body) = block(
  width: 100%, fill: lightgold, stroke: 1pt + gold, radius: 3pt, inset: 8pt,
  [#text(fill: gold, weight: "bold", size: 9.5pt)[Speed Trick] #v(2pt) #body]
)

#let trapbox(body) = block(
  width: 100%, fill: lightred, stroke: 1pt + red, radius: 3pt, inset: 8pt,
  [#text(fill: red, weight: "bold", size: 9.5pt)[Time Trap] #v(2pt) #body]
)
...
#let mcq(a, b, c, d, e) = block(
  inset: (left: 12pt, y: 2pt),
  text(size: 9.5pt)[(A) #a #h(1.5em) (B) #b #h(1.5em) (C) #c #h(1.5em) (D) #d #h(1.5em) (E) #e]
)
...
// Speed solution: shows the fast method explicitly
#let speedsolution(time, body) = block(
  width: 100%, stroke: (left: 2.5pt + gold), inset: (left: 7pt, y: 3pt),
  [#text(size: 9pt)[*Speed Solution* #text(fill: gold, weight: "bold")[(#time)]*:* #body]]
)

// Standard solution for comparison
#let solution(body) = block(
  width: 100%, stroke: (left: 2.5pt + green), inset: (left: 7pt, y: 3pt),
  [#text(size: 9pt)[*Textbook Solution.* #body]]
)
...
#let drill-header(title) = {
  ...
  block(
    ...
    text(fill: gold, weight: "bold", size: 10pt)[SPEED DRILL — #title]
  )
}

// Difficulty by question position
#let difficulty(level) = {
  let label = if level == 1 { "Q1–Q7" } else if level == 2 { "Q8–Q14" } else { "Q15–Q20" }
  let desc = if level == 1 { "Quick" } else if level == 2 { "Medium" } else { "Hard" }
  let col = if level == 1 { green } else if level == 2 { gold } else { red }
  text(fill: col, size: 8.5pt, weight: "bold")[#desc (#label)]
}
...
#page(header: none, footer: none, margin: (top: 1.8in, bottom: 1in))[
  #align(center)[
    #block(width: 85%, fill: navy, inset: (x: 24pt, y: 28pt), radius: 8pt)[
      #text(fill: gold, size: 28pt, weight: "bold")[NYSMATYC]
      #v(2pt)
      #text(fill: white, size: 18pt)[Speed Handbook]
      #v(10pt)
      #line(length: 50%, stroke: 1pt + gold)
      #v(10pt)
      #text(fill: gold, size: 12pt, weight: "bold")[OLYMPIAD EDGE]
      #v(4pt)
      #text(fill: white, size: 10pt)[Fastest Solutions · Every Problem Type]
    ]
    #v(10pt)
    #block(width: 80%, inset: 12pt)[
      #text(size: 10.5pt, fill: navy)[
        Speed-first approach to the NYSMATYC Math League. \
        Every problem type gets the *fastest path* — not the textbook method. \
        Back-solving, estimation, elimination, and pattern recognition.
        #v(8pt)
        *20 questions · 60 minutes · Multiple choice (A–E)*\
        *+4 correct · --0.5 wrong · 0 blank*
        #v(6pt)
        #text(size: 9pt, fill: luma(100))[
          Based on NYSMATYC and AMATYC Student Mathematics League exams 2000–2025. \
          Precalculus level — no calculus required.
        ]
      ]
    ]
    #v(12pt)
