...
#set document(title: "Algebra II — Olympiad Edge", author: "Olympiad Edge")
#set page(
  paper: "us-letter",
  ...
  header: context {
    if counter(page).get().first() > 2 {
      ...
      grid(
        columns: (1fr, 1fr),
        align(left)[_Olympiad Edge — Algebra II_],
...
#let difficulty(level) = {
  ...
  let label = if level == 1 { "AMC 10" } else if level == 2 { "AMC 12" } else { "AIME" }
...
#page(header: none, footer: none, margin: (top: 1.8in, bottom: 1in))[
  #align(center)[
    #block(width: 85%, fill: navy, inset: (x: 24pt, y: 28pt), radius: 8pt)[
      #text(fill: gold, size: 30pt, weight: "bold")[ALGEBRA II]
      #v(2pt)
      ...
      #v(4pt)
      #text(fill: white, size: 10pt)[AMC 10 · AMC 12 · AIME · Self-Study Edition]
    ]
    #v(10pt)
    #block(width: 80%, inset: 12pt)[
      #text(size: 10.5pt, fill: navy)[
        Pattern recognition first — theory serves the problem. \
        Every theorem gets a one-line TLDR. Escalating hints on every problem. \
        Competition tricks and traps flagged explicitly.
        #v(8pt)
        *Target: AMC 12 Honor Roll / AIME Qualification*
        #v(6pt)
        #text(size: 9pt, fill: luma(100))[
          Sources: Art of Problem Solving, Evan Chen, Brilliant.org, \
          past AMC 10/12 and AIME problems 2000–2025, Engel (Problem-Solving Strategies).
        ]
      ]
    ]
    #v(12pt)
